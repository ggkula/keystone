...
var searchreplace = (function () {
  ...
  var $_6xbhc4ihjcfx5cie = { findAndReplaceDOMText: findAndReplaceDOMText };
  ...
  var markAllMatches = function (editor, currentIndexState, regex) {
    ...
    return $_6xbhc4ihjcfx5cie.findAndReplaceDOMText(regex, node, marker, false, editor.schema);
  };
  ...
  var get = function (editor, currentIndexState) {
    var done = function (keepEditorSelection) {
      return $_btzbbuifjcfx5ci7.done(editor, currentIndexState, keepEditorSelection);
    };
    var find = function (text, matchCase, wholeWord) {
      return $_btzbbuifjcfx5ci7.find(editor, currentIndexState, text, matchCase, wholeWord);
    };
    var next = function () {
      return $_btzbbuifjcfx5ci7.next(editor, currentIndexState);
    };
    var prev = function () {
      return $_btzbbuifjcfx5ci7.prev(editor, currentIndexState);
    };
    var replace = function (text, forward, all) {
      return $_btzbbuifjcfx5ci7.replace(editor, currentIndexState, text, forward, all);
    };
    return {
      done: done,
      find: find,
  ...
  var open = function (editor, currentIndexState) {
    ...
    function updateButtonStates() {
      win.statusbar.find('#next').disabled($_btzbbuifjcfx5ci7.hasNext(editor, currentIndexState) === false);
      win.statusbar.find('#prev').disabled($_btzbbuifjcfx5ci7.hasPrev(editor, currentIndexState) === false);
    }
    ...
    var win = editor.windowManager.open({
      ...
      onClose: function () {
        editor.focus();
        $_btzbbuifjcfx5ci7.done(editor, currentIndexState);
        editor.undoManager.add();
      },
      onSubmit: function (e) {
        ...
        if (!text.length) {
          $_btzbbuifjcfx5ci7.done(editor, currentIndexState, false);
          win.statusbar.items().slice(1).disabled(true);
          return;
        }
        if (last.text === text && last.caseState === caseState && last.wholeWord === wholeWord) {
          if (!$_btzbbuifjcfx5ci7.hasNext(editor, currentIndexState)) {
            notFoundAlert();
            return;
          }
          $_btzbbuifjcfx5ci7.next(editor, currentIndexState);
          updateButtonStates();
          return;
        }
        count = $_btzbbuifjcfx5ci7.find(editor, currentIndexState, text, caseState, wholeWord);
        if (!count) {
          notFoundAlert();
        }
      ...
      buttons: [
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            if (!$_btzbbuifjcfx5ci7.replace(editor, currentIndexState, win.find('#replace').value())) {
              win.statusbar.items().slice(1).disabled(true);
              currentIndexState.set(-1);
              last = {};
            }
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_btzbbuifjcfx5ci7.replace(editor, currentIndexState, win.find('#replace').value(), true, true);
            win.statusbar.items().slice(1).disabled(true);
            last = {};
          }
        },
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_btzbbuifjcfx5ci7.prev(editor, currentIndexState);
            updateButtonStates();
          }
        },
        {
  ...
  var $_emraxvijjcfx5cil = { open: open };

  var register = function (editor, currentIndexState) {
    editor.addCommand('SearchReplace', function () {
      $_emraxvijjcfx5cil.open(editor, currentIndexState);
    });
  };
  var $_3qn270iijcfx5cij = { register: register };
  ...
  var $_dvrextikjcfx5cio = { register: register$1 };

  PluginManager.add('searchreplace', function (editor) {
    var currentIndexState = Cell(-1);
    $_3qn270iijcfx5cij.register(editor, currentIndexState);
    $_dvrextikjcfx5cio.register(editor, currentIndexState);
    return $_7mrpyriejcfx5ci4.get(editor, currentIndexState);
  });
  var Plugin = function () {
  };
